...
단어사전은 `words.html`에서 열리며, 첫 화면에서는 `data/words/meta.json`만 읽고 검색어 첫 글자에 해당하는 `data/words/*.json` 조각 파일만 추가로 불러옵니다. 단어 데이터는 초급·중급 중심으로 줄이고, 접사·어미·조사·문장형 표현·깨진 대역 표기는 제외합니다.
