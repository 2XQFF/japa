...
단어사전은 `words.html`에서 열리며, 첫 화면에서는 `data/words/meta.json`만 읽고 검색어 첫 글자에 해당하는 `data/words/*.json` 조각 파일만 추가로 불러옵니다. 단어 데이터는 초급·중급 중심으로 줄이고, 접사·어미·조사·문장형 표현·깨진 대역 표기는 제외합니다. 뜻은 항목당 최대 3개만 남기고 난이도와 형태를 기준으로 정렬하며, 검색 결과는 일치도·뜻 적합도·난이도·읽기순을 함께 반영합니다.
